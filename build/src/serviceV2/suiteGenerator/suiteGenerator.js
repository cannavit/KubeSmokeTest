...
function addDependencies(options) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        let suiteSmokeTest = "";
        let dependencies = yield fs.promises.readFile(dependenciesFile, "utf8", 
        // @ts-ignore
        function (err, data) { });
        for (const criterial of Object.keys(options.smokeTestSuites)) {
            for (const test of Object.keys(options.smokeTestSuites[criterial])) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const criterial of Object.keys(options.smokeTestSuites)) {
            for (const test of Object.keys(options.smokeTestSuites[criterial])) {
                ...
                try {
                    swaggerLoginCurl =
                        options.smokeTestSuites["--endpoint-coverage"]["--swagger-login-curl"];
                }
                catch (error) { }
                if (swaggerLoginCurl) {
                ...
                if (listOfTestPath[test] === undefined) {
                    throw new Error(chalk.red.bold(` 🛑  The Input: ${test} is required inside of the list listOfTestPath`));
                }
                const testPath = __dirname + listOfTestPath[test];
                // $$criterial
                let testType = Object.keys(smktest.testType)[0];
                // @ts-ignore
                let testContent = yield fs.promises.readFile(testPath, "utf8", 
                // @ts-ignore
                function (err, data) { });
                // Replace variables
                ...
                if (testType == "swaggerDocs") {
                    ...
                    if (swaggerLoginCurl) {
                        ...
                        for (const swagger of responseOfRequest) {
                            ...
                            // );
                            testContentElement = yield replaceAll(testContentElement, "$$curlLogin", "false" //TODO only for test
                            );
                    ...
                    var tasksInit = new Listr([
                        {
                            ...
                            task: () => __awaiter(this, void 0, void 0, function* () {
                                // Add Dependencies File.
                                yield fs.promises.writeFile(pathTestOne, smktest.textDependencies, 
                                // @ts-ignore
                                function (err) { });
                            })
                        }
