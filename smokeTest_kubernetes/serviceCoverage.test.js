...
test('Smoke Test criterial --service-coverage test name: --check-pods-logs', async () => {
  ...
  let reportCommand = 'kubectl logs --since=1h service/$$serviceName --namespace=edutelling-develop | tail -4';
  let testCommand = 'kubectl logs --since=1h service/$$serviceName --namespace=edutelling-develop | grep -i "error"';
  let assertValue = '';
  let environmentVariableResultTest = '$$environmentVariableResultTest';
