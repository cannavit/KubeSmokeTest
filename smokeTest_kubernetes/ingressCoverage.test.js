...
test('Smoke Test $${criterial}test name: --check-ingress', async () => {
  ...
  let testCommand = 'curl -v http://edutelling-db-develop.openshift.techgap.itundefined  2>&1 | grep "ERROR"';
  let assertValue = '';
...
test('Smoke Test $${criterial}test name: --check-ingress', async () => {
  ...
  let criterial = '--ingress-coverage';
  let consoleValue = '--check-ingress';
  let testCommand = 'curl -v http://edutelling-api-develop.openshift.techgap.it/api  2>&1 | grep "ERROR"';
  let assertValue = '';
  let reportCommand = 'curl -v $ingress  2>&1';
...
test('Smoke Test $${criterial}test name: --check-ingress', async () => {
  ...
  let criterial = '--ingress-coverage';
  let consoleValue = '--check-ingress';
  let testCommand = 'curl -v http://edutelling-app-develop.openshift.techgap.it/  2>&1 | grep "ERROR"';
  let assertValue = '';
  let reportCommand = 'curl -v $ingress  2>&1';
